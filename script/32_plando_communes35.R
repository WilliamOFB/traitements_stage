plando_35 <- plando %>%
  st_intersection(communes35) %>% # découpage des plans d'eau selon les communes
...
affectation_commune35 <- plando_35 %>% 
...
plando_35 <- plando %>% 
  left_join(y = affectation_commune35) %>% 
  select(NatureSE,
         OrigineSE,
         Persistanc,
         surface_plando,
...
save(plando_35,
     file = "../traitements_stage/processed_data/plando35.RData")

load("../traitements_stage/processed_data/communes29.RData")

plando <- plando %>% 
  sf::st_make_valid() # rendre les plando valides (essai 1)

plando <- plando %>% 
  fortify() # rendre les plando valides (essai 2)

plando_29 <- plando %>%
  st_intersection(communes29) %>% # découpage des plans d'eau selon les communes
...
affectation_commune29 <- plando_29 %>% 
...
plando_29 <- plando %>% 
  left_join(y = affectation_commune29) %>% 
  select(NatureSE,
         OrigineSE,
         Persistanc,
         surface_plando,
...
save(plando_29,
     file = "../traitements_stage/processed_data/plando29.RData")

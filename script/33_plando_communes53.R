plando_53 <- plando %>%
  st_intersection(communes53) %>% # découpage des plans d'eau selon les communes
...
affectation_commune53 <- plando_53 %>% 
...
plando_53 <- plando %>% 
  left_join(y = affectation_commune53) %>% 
  select(NatureSE,
         OrigineSE,
         Persistanc,
         surface_plando,
...
save(plando_53,
     file = "../traitements_stage/processed_data/plando53.RData")
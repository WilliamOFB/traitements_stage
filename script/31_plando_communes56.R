...
plando_56 <- plando %>%
  st_intersection(communes56) %>% # découpage des plans d'eau selon les communes
...
affectation_commune56 <- plando_56 %>% 
...
plando_56 <- plando %>% 
  left_join(y = affectation_commune56) %>% 
  select(NatureSE,
         OrigineSE,
         Persistanc,
         surface_plando,
...
save(plando_56,
     file = "../traitements_stage/processed_data/plando56.RData")
